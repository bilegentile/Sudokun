...
final class Grid: ObservableObject {
    ...
    @ObservedObject var sudokuVM: SeedsViewModel = SeedsViewModel()
    @Published private var sudoku: Puzzles? = Puzzles.emptySudoku
    ...
    func loadFromSeed() async -> Void {
		let randPuzzle: Int = Int.random(in: 0 ..< Puzzles.easy.count)
        self.difficulty = "Easy"
        let downloadSudoku = Task(priority: .high) { () -> Puzzles in
            do {
        ...
        let result = await downloadSudoku.result
        
        do {
            let sudoku = try result.get()
            let strTask = sudoku.task.map { row in
                row.map { String($0) }.joined()
            }.joined()
            print("\(strTask)")
            var str: String = strTask
            var count: Int = 0
